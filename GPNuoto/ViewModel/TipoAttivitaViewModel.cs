...
namespace GPNuoto.ViewModel
{
    ...
    /// </para>
    /// </summary>
    public class TipoAttivitaViewModel : ViewModelBase
    {
        /// <summary>
        ...
        bool CheckForSave()
        {
            if (_titolo.Trim().Length > 0 && _codiceContabileCassa.Length>0)
            {
                if (IsNew)
                    return !dataservice.IsTipoAttivita(_titolo.Trim());
                else
                    return true;
            }
            else return false;
        ...
        public bool IsSelezionata
        {
            ...
            set
            {
                ...
                SimpleIoc.Default.GetInstance<CalendarManagerViewModel>().ChangedTipoAttivita();
        ...
        private RelayCommand _editTipoAttivita;

        /// <summary>
        /// Gets the EditTipoAttivita.
        /// </summary>
        public RelayCommand EditTipoAttivita
        {
            get
            {
                return _editTipoAttivita
                    ?? (_editTipoAttivita = new RelayCommand(
                    () =>
                    {
                        TableTipoAttivitaViewModel tavm = SimpleIoc.Default.GetInstance<TableTipoAttivitaViewModel>();
                        tavm.ElementoSelezionato = this;
                        tavm.OpenEdit.Execute(null);
                    }));
            }
        }
